...
def get_scatter(entered_site,slider_range):
    min = slider_range[0],
    max = slider_range[1],
    filtered_df2 = spacex_df[(spacex_df['Payload Mass (kg)'] > min) & (spacex_df['Payload Mass (kg)'] < max)]
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df2, x='Payload Mass (kg)', y='class',
        color='Booster Version Category',title='Total Payload Mass per Class')
        return fig
    else:
        filtered_df3 = filtered_df2[filtered_df2['Launch Site'] == entered_site]
        fig2 = px.scatter(filtered_df3, x='Payload Mass (kg)', y='class',
        color='Booster Version Category',
        title=f'Payload Mass per Class for {entered_site}')
        return fig2
# Run the app
